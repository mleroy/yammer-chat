    ...
    public class BayeuxClient : AbstractClientSession, IBayeux
    {
        ...
        protected void processHandshake(IMutableMessage handshake)
        {
            if (handshake.Successful)
            {
                ...
                foreach (var serverTransport in serverTransportObject as IEnumerable<Object>)
                {
                   serverTransports.Add(serverTransport.ToString());
                }
        ...
        private bool scheduleAction(EventHandler action, long interval, long backoff)
        {
            long wait = interval + backoff;
            if (wait <= 0) wait = 1;

            Timer timer = new Timer(new TimerCallback((x) => { action(this, null); }), null, (int)wait, 0); // @@ax: What about support for multiple timers?
